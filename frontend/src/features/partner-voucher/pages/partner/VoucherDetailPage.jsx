...
export function VoucherDetailPage() {
    ...
    return (
      <PartnerLayout>
        <div className="p-12 text-center text-slate-500">
          {t("Đang tải thông tin chi tiết Voucher...")}
        </div>
      </PartnerLayout>
    );
  }
  ...
  if (!voucher) {
    return (
      <PartnerLayout>
        <div className="p-12 text-center text-slate-500">
          {t("Không tìm thấy thông tin Voucher.")}
        </div>
      </PartnerLayout>
    );
  }
  ...
  const handleStatusChange = async (newStatus, msg) => {
    const reviewStatus = ["Dang ban", "Tam ngung", "Ngung ban"].includes(
      newStatus,
    )
      ? "Da duyet"
      : newStatus === "Cho duyet"
  ...
  const isRejected =
    voucher.trang_thai === "Tu choi" ||
    voucher.trang_thai_kiem_duyet === "Tu choi";
            ...
            {isTamNgung && (
              <>
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    handleStatusChange("Dang ban", t("Đã mở bán lại Voucher!"))
                  }
                >
                  {t("Mở bán lại")}
                </Button>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                <h4 className="font-bold text-rose-900 text-sm">
                  {t("Voucher này bị Quản trị viên từ chối phê duyệt")}
                </h4>
                <p className="text-xs text-rose-700 font-medium">
                  {t("Lý do từ chối:")}{" "}
                  <span className="italic font-normal">
                    {voucher.ly_do_tu_choi ||
                      t("Chưa đáp ứng điều kiện niêm yết giá hoặc chi nhánh.")}
                  </span>
                </p>
              </div>
            </div>
          ...
          <div className="relative h-64 w-full bg-slate-900 overflow-hidden">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-slate-950/80 to-transparent text-white">
              ...
              <p className="text-xs text-slate-300 mt-1">
                {t("Mã hệ thống:")} {voucher.ma_voucher}
              </p>
            </div>
          </div>
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-1">
              ...
              <div className="text-2xl font-bold text-emerald-600">
                {voucher.gia_ban?.toLocaleString()}đ
              </div>
              <div className="text-xs text-slate-400 line-through">
                {t("Giá gốc:")} {voucher.gia_goc?.toLocaleString()}đ
              </div>
            </div>
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-1">
              ...
              <div className="text-xs font-bold text-slate-900 mt-1">
                {voucher.tg_bat_dau_ban
                  ? new Date(voucher.tg_bat_dau_ban).toLocaleDateString("vi-VN")
                  : "-"}{" "}
                -{" "}
                {voucher.tg_ket_thuc_ban
                  ? new Date(voucher.tg_ket_thuc_ban).toLocaleDateString(
                      "vi-VN",
                    )
                  : "-"}
              </div>
              <div className="text-xs text-slate-500">
                {t("Áp dụng trong khung giờ mở cửa")}
              </div>
            </div>
          </div>
        </Card>
          ...
          <Card title={t("Điều Kiện & Mô Tả")}>
            <div className="space-y-3 text-xs text-slate-700">
              ...
              <div>
                ...
                <p>
                  {voucher.dieu_kien_ap_dung ||
                    t("Theo quy định của hệ thống.")}
                </p>
              </div>
              <div>
                <strong className="block text-slate-900 mb-1">
                ...
                <p>
                  {voucher.chinh_sach_hoan_huy ||
                    t("Theo chính sách của đối tác.")}
                </p>
              </div>
            </div>
          </Card>
          ...
          <Card title={t("Chi Nhánh Áp Dụng")}>
            {activeBranches.length === 0 ? (
              <p className="text-xs text-slate-400">
                {t("Áp dụng trên toàn bộ chi nhánh đang hoạt động.")}
              </p>
            ) : (
        ...
        {isNgungBan && (
          <div className="bg-rose-50 border border-rose-200 rounded-xl p-5 shadow-xs">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                <h4 className="font-bold text-rose-900 text-sm">
                  {t("Voucher này đã ngừng bán vĩnh viễn")}
                </h4>
                <p className="text-xs text-rose-700 font-medium">
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4 text-slate-800">
              <div className="flex items-center gap-3 text-rose-600">
                ...
                <h3 className="font-bold text-slate-900 text-lg">
                  {t("Xác nhận NGỪNG BÁN Voucher")}
                </h3>
              </div>

              <div className="bg-rose-50 border border-rose-200 rounded-xl p-4 text-xs text-rose-900 space-y-2">
                <p className="font-bold">
                  {t("Chương trình Voucher:")} {voucher.ten_voucher}
                </p>
                <p className="leading-relaxed">
                  {t(
                    "Khi xác nhận Ngừng bán, Voucher này sẽ bị đóng vĩnh viễn",
                  )}
                </p>
              </div>
                ...
                <button
                  onClick={async () => {
                    ...
                    await handleStatusChange(
                      "Ngung ban",
                      t(
                        "Voucher đã được chuyển sang trạng thái NGỪNG BÁN vĩnh viễn!",
                      ),
                    );
                  }}
        ...
        <Modal
          ...
          onConfirm={async () => {
            ...
            await handleStatusChange(
              "Cho duyet",
              t(
                "Đã gửi yêu cầu xét duyệt Voucher tới Quản trị viên thành công!",
              ),
            );
          }}
        ...
        >
          <div className="space-y-3 text-left">
            <p className="text-sm text-slate-700">
              {t("Bạn có chắc chắn muốn gửi chương trình Voucher")}{" "}
