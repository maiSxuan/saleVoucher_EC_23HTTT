...
class OrderRepository {
  ...
  async cancelOrder(maDh, reason) {
    const { data: oldDh } = await supabase.from('donhang').select('trang_thai').eq('ma_dh', maDh).single();
    if (!oldDh) throw new Error('Không tìm thấy đơn hàng');

    const operations = [
      {
        execute: async (tx) => {
          const { data, error } = await tx.from('donhang').update({ trang_thai: OrderStatus.CHO_HOAN_TIEN, ly_do_huy: reason }).eq('ma_dh', maDh).select().single();
          if (error) throw new Error(`Lỗi hủy đơn hàng: ${error.message}`);
          return data;
        },
        rollback: async (tx) => {
          await tx.from('donhang').update({ trang_thai: oldDh.trang_thai, ly_do_huy: null }).eq('ma_dh', maDh);
        }
      }
    ];
  ...
  async confirmRefund(maDh, reason, maTkAdmin) {
    const { data: oldDh } = await supabase.from('donhang').select('trang_thai, ly_do_huy').eq('ma_dh', maDh).single();
    const { data: oldVouchers } = await supabase.from('voucher_mua').select('ma_voucher_mua, trang_thai').eq('ma_dh', maDh);
    const { data: pays } = await supabase.from('thanhtoan').select('*').eq('ma_dh', maDh).eq('trang_thai', PaymentStatus.THANH_CONG).order('thoi_gian_tt', { ascending: false }).limit(1);
    const latestPay = pays && pays[0] ? pays[0] : null;

    const operations = [
      {
        execute: async (tx) => {
          const { error } = await tx.from('donhang').update({ trang_thai: OrderStatus.DA_HOAN_TIEN, ly_do_huy: reason }).eq('ma_dh', maDh);
          if (error) throw error;
        },
        rollback: async (tx) => {
          if (oldDh) await tx.from('donhang').update({ trang_thai: oldDh.trang_thai, ly_do_huy: oldDh.ly_do_huy }).eq('ma_dh', maDh);
        }
      },
      {
        execute: async (tx) => {
          const { error } = await tx.from('voucher_mua').update({ trang_thai: VoucherCodeStatus.VO_HIEU_HOA }).eq('ma_dh', maDh);
          if (error) throw error;
        },
        rollback: async (tx) => {
          if (oldVouchers && oldVouchers.length > 0) {
            for (const v of oldVouchers) {
               await tx.from('voucher_mua').update({ trang_thai: v.trang_thai }).eq('ma_voucher_mua', v.ma_voucher_mua);
            }
          }
        }
      }
      ...
      operations.push({
        execute: async (tx) => {
          const { data, error } = await tx.from('hoantien').insert({
            ...
            trang_thai: PaymentStatus.THANH_CONG,
            ly_do: reason,
  ...
  async rejectRefund(maDh, reason, maTkAdmin) {
    ...
    const operations = [
      {
        execute: async (tx) => {
          const { error } = await tx.from('donhang').update({ trang_thai: OrderStatus.HUY_YEU_CAU_HOAN_TIEN, ly_do_huy: reason }).eq('ma_dh', maDh);
          if (error) throw error;
        },
        rollback: async (tx) => {
          if (oldDh) await tx.from('donhang').update({ trang_thai: oldDh.trang_thai, ly_do_huy: oldDh.ly_do_huy }).eq('ma_dh', maDh);
    ...
    if (latestPay) {
      operations.push({
        execute: async (tx) => {
          const { data, error } = await tx.from('hoantien').insert({
            so_tien: latestPay.so_tien,
            trang_thai: PaymentStatus.THAT_BAI,
            ly_do: reason,
            ngay_xu_ly: new Date().toISOString(),
            ma_tk: maTkAdmin,
            ma_thanh_toan: latestPay.ma_thanh_toan,
  ...
  async reissueVoucherCode(maVoucherMua, maTkAdmin) {
    ...
    const operations = [
      {
        execute: async (tx) => {
          const { error } = await tx.from('voucher_mua').update({ trang_thai: VoucherCodeStatus.VO_HIEU_HOA }).eq('ma_voucher_mua', maVoucherMua);
          if (error) throw error;
        },
        rollback: async (tx) => {
