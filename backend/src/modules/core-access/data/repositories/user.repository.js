...
class UserRepository {
  ...
  async findAccountByLoginInfo(loginInfo) {
    ...
    try {
      ...
      console.error(
        "[UserRepository] findAccountByLoginInfo error:",
        e.message,
      );
      return null;
    }
  ...
  async findAllBranches({ maHsdn, includeInactive = false } = {}) {
    let query = supabase
      .from("chinhanh")
      .select(
        "ma_chi_nhanh, ten_chi_nhanh, dia_chi, khu_vuc, trang_thai, ma_hs",
      )
  ...
  async findBranchById(branchId) {
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error)
      throw new Error(`Không thể kiểm tra chi nhánh: ${error.message}`);
    return data;
  }
  ...
  async getUserBranchInfo(maChiNhanh) {
    ...
    const { data, error } = await supabase
      .from("chinhanh")
      .select(
        "ma_chi_nhanh, ten_chi_nhanh, dia_chi, ma_hs, hosodn:ma_hs(ten_dn)",
      )
  ...
  async updateStatus(userId, newStatus) {
    ...
    if (error) {
      ...
      throw new Error(
        `Cập nhật trạng thái người dùng thất bại: ${error.message}`,
      );
    }
    return data;
  }
  ...
  async updateRole(userId, newRole, maChiNhanh = null, maHsdn = null) {
    ...
    if (newRole === "Nhan vien ban hang") {
      ...
      newRole === "Nhan vien quan ly voucher" ||
      newRole === "Nguoi dai dien"
    ) {
