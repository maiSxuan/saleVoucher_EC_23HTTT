...
- `Tạm khóa` — trạng thái quản trị do Admin áp dụng.
...
| Tạm khóa | Không tạo phiên | Giữ nguyên, không được truy cập khi chưa mở khóa |
...
- Tài khoản đã `Tạm khóa` hoặc `Tạm khóa` → từ chối đăng nhập.
...
Figma prototype phải thể hiện nút Tạm khóa khi đang xử lý. Backend triển khai thật phải dùng khóa/idempotency để:
