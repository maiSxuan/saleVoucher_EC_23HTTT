...
# BÁO CÁO RÀ SOÁT UC-ADM-04 — QUẢN LÝ ĐƠN HÀNG

**Ngày rà soát:** 11/08/2026

**Tài liệu nguồn:** `docs/đặc tả hệ thống của admin.pdf`, mục `UC-ADM-04`, trang 21–29 của PDF.

**Phạm vi:** Chỉ đọc và đối chiếu đặc tả với code hiện tại; không sửa logic ứng dụng trong lần rà soát này.

## 1. Kết luận ngắn

**UC-ADM-04 hiện CHƯA KHỚP ĐẦY ĐỦ với đặc tả và CHƯA ĐỦ ĐIỀU KIỆN NGHIỆM THU.**

Phần khung đã có gồm: route dành cho Admin, màn hình danh sách/chi tiết, lịch sử thanh toán, khu vực lịch sử mã, API hủy/hoàn tiền/từ chối hoàn tiền/cấp lại mã và một số audit log. Tuy nhiên, các luồng nghiệp vụ quan trọng hiện có lỗi hoặc còn thiếu:

1. Backend/UI dùng bộ trạng thái tiếng Anh khác với CHECK constraint của database.
2. Hoàn tiền, vô hiệu hóa mã, cấp lại mã và ghi audit không chạy trong cùng transaction.
3. API không kiểm tra đầy đủ tiền điều kiện trước khi đổi trạng thái.
4. Cấp lại mã chưa gửi email và chưa lưu kết quả gửi.
5. Giao diện Admin chưa có nút/modal xử lý yêu cầu hủy đơn dù có state và API.
6. Tìm kiếm được thực hiện sau phân trang nên có thể thiếu kết quả và sai tổng số.
7. Audit log thiếu thao tác kiểm tra đơn, trạng thái trước/sau, mã cũ/mới và kết quả gửi lại mã.

Vì các lỗi trên tác động trực tiếp tới trạng thái đơn hàng, thanh toán và voucher code, không nên đánh dấu UC này là “đã hoàn thành” chỉ vì các endpoint và màn hình đã tồn tại.

## 2. Code thực tế đang được sử dụng

Luồng được mount và được frontend gọi thật là:

```text
Frontend /admin/orders
  -> frontend/src/features/order/pages/admin/AdminOrdersPage.jsx
  -> frontend/src/features/order/api/orderApi.js
  -> /api/admin/orders...
  -> backend/src/modules/order/presentation/routes/admin-order.routes.js
  -> backend/src/modules/order/presentation/controllers/order.controller.js
  -> backend/src/modules/order/business/services/order.service.js
  -> backend/src/modules/order/data/repositories/order.repository.js
  -> Supabase/PostgreSQL
```

Dẫn chứng:

- `frontend/src/app/router.jsx:183-196` gắn `AdminOrdersPage` vào `/admin/orders` và bọc `ProtectedRoute` cho Admin.
- `backend/src/routes/index.js:19-23` đăng ký `orderModule`.
- `backend/src/modules/order/index.js:5-7` mount route customer và admin.
- `backend/src/modules/order/presentation/routes/admin-order.routes.js:9-70` khai báo các API Admin và đều có middleware xác thực/phân quyền.

Lưu ý: bộ file `backend/src/modules/customer-commerce/presentation/routes/admin-order.routes.js` chỉ là code placeholder cũ. `customer-commerce/index.js:17-23` không mount route admin-order đó, nên nó không phải logic đang chạy.

## 3. Lỗi nghiêm trọng cần xử lý trước

### 3.1. Sai bộ trạng thái giữa code và database — mức SAI/CHẶN

Database hiện định nghĩa:

- Thanh toán: `Dang xu ly`, `Thanh cong`, `That bai` tại `database/create_tables.sql:307-315`.
- Hoàn tiền: `Dang xu ly`, `Thanh cong`, `That bai` tại `database/create_tables.sql:321-331`.
- Voucher code: `Chua su dung`, `Da su dung`, `Het han`, `Loi sinh ma`, `Vo hieu hoa` tại `database/create_tables.sql:338-357`.
- Dữ liệu seed cũng sử dụng đúng các giá trị này, ví dụ `Thanh cong`, `Chua su dung`, `Vo hieu hoa` tại `database/seeds.sql:884-1038`.

Trong khi đó module order lại dùng:

- `success`, `failed`, `refunded_sim` cho thanh toán.
- `completed`, `rejected` cho hoàn tiền.
- `issued`, `generation_error`, `disabled` cho voucher code.

Dẫn chứng tại `backend/src/modules/order/data/repositories/order.repository.js:490-523`, `540-578`, `581-604` và `frontend/src/features/order/pages/admin/AdminOrdersPage.jsx:73-89`, `229-231`.

Hậu quả nếu database được tạo theo script hiện tại:

- Xác nhận thanh toán thủ công gửi `success` sẽ bị CHECK constraint của `THANHTOAN` từ chối.
- Mã có trạng thái thật `Loi sinh ma` không khớp `generation_error`, nên UI không nhận ra lỗi và không bật đúng nút cấp lại.
- Hoàn tiền cập nhật voucher thành `disabled`, tạo hoàn tiền `completed` và đổi payment thành `refunded_sim`; cả ba đều không thuộc bộ trạng thái của schema.
- Từ chối hoàn tiền ghi `rejected`, cũng không thuộc CHECK constraint của `HOANTIEN`.
- Cấp lại mã tạo voucher mới với `issued`, không thuộc CHECK constraint của `VOUCHER_MUA`.
- UI nhận `Thanh cong` nhưng hàm badge chỉ nhận `success`, vì vậy giao dịch thành công có thể bị hiển thị như “Chờ”.

Đây là lỗi chặn trực tiếp các luồng A14b và A14c, không phải lỗi hiển thị nhỏ.

### 3.2. Không có transaction và nhiều lỗi database bị bỏ qua — mức SAI/CHẶN

Các thao tác UC-ADM-04 là nhiều câu lệnh độc lập:

- Hoàn tiền đổi đơn sang `Da hoan tien` trước, sau đó mới vô hiệu mã, tạo `hoantien`, rồi đổi trạng thái thanh toán tại `order.repository.js:540-557`.
- Cấp lại mã vô hiệu mã cũ, tạo mã mới, sau đó mới ghi `lssinhma` tại `order.repository.js:581-612`.
- Xác nhận thanh toán tạo payment, đổi đơn và sinh nhiều mã ở các lệnh riêng tại `order.repository.js:490-523`.
- Sau khi repository thay đổi dữ liệu xong, service mới ghi audit log tại `order.service.js:80-153`.

Nhiều lệnh Supabase trong các đoạn trên không kiểm tra thuộc tính `error`. Ví dụ `confirmRefund()` luôn trả `{ success: true }` tại dòng 559 dù một hoặc nhiều cập nhật trước đó có thể thất bại.

`auditLogService.log(..., true)` chỉ ném lỗi nếu ghi log thất bại (`audit-log.service.js:50-54`), nhưng không rollback được các cập nhật đã thực hiện trước đó. Điều này trái trực tiếp với:

- Ngoại lệ 2, 4, 5 và 8 của UC.
- NFR-03: không được cập nhật một phần, không báo thành công nếu dữ liệu/log chưa lưu đầy đủ.

### 3.3. Thiếu kiểm tra tiền điều kiện tại backend — mức SAI/CHẶN

Controller lấy dữ liệu body rồi gọi service ngay, không có validator nghiệp vụ (`order.controller.js:87-160`). Repository cũng không khóa hoặc kiểm tra trạng thái hiện tại trước khi update.

Các trường hợp hiện có thể gọi trực tiếp qua API:

- Hủy bất kỳ đơn nào và chuyển thẳng sang `Cho hoan tien`, không kiểm tra đã thanh toán thành công và chưa phát hành code (`order.repository.js:528-537`).
- Hoàn tiền một đơn không ở trạng thái `Cho hoan tien` (`order.repository.js:540-559`).
- Từ chối hoàn tiền cho một đơn không có yêu cầu hoàn tiền (`order.repository.js:562-578`).
- Gửi `reason` rỗng vì kiểm tra lý do chỉ nằm ở frontend, không nằm ở API.
- Gửi `newStatus` tùy ý vào API xác nhận thanh toán.
- Cấp lại `maVoucherMua` không thuộc đơn hàng trên URL. `order.service.js:141-142` bỏ qua `orderId` và repository chỉ tra theo `maVoucherMua`.
- Cấp lại mã mà không kiểm tra payment thật sự là `Thanh cong` và mã thật sự là `Loi sinh ma`.

Việc ẩn/hiện nút ở frontend không thay thế được kiểm tra tại backend theo NFR-02 của đặc tả.

### 3.4. Luồng cấp lại mã chưa gửi email và chưa có lịch sử gửi — mức THIẾU

`reissueVoucherCode()` chỉ:

1. Đọc mã cũ.
2. Update mã cũ.
3. Sinh/insert mã mới.
4. Insert `lssinhma`.
5. Trả mã mới.

Đoạn này kết thúc tại `order.repository.js:581-614`; không có lời gọi mailer/email service.

Ngoài ra, bảng `LSSINHMA` chỉ có mã cũ, mã mới, thời gian, mã voucher mua và Admin thực hiện (`database/create_tables.sql:364-372`); không có thông tin lần gửi, kênh gửi hoặc kết quả gửi thành công/thất bại.

Vì vậy các yêu cầu sau chưa có:

- Tự động gửi mã mới cho khách hàng qua email.
- Ghi lịch sử gửi mã và kết quả gửi.
- Ngoại lệ “mã đã xử lý nhưng chưa thể gửi”.
- Không sinh thêm mã khi chỉ gửi email thất bại.

### 3.5. Luồng xử lý yêu cầu hủy chưa hoàn chỉnh — mức THIẾU/MỘT PHẦN

Backend có `POST /admin/orders/:id/cancel`, nhưng giao diện không render nút/modal để Admin thực hiện:

- Có state `cancelModal` tại `AdminOrdersPage.jsx:113`.
- Có hàm `handleCancelOrder()` tại `AdminOrdersPage.jsx:191-203`.
- Không có lời gọi `setCancelModal(true)` và không có JSX modal hủy trong phần modal `AdminOrdersPage.jsx:437-499`.

Luồng customer tại `order.controller.js:111-124` lại đổi đơn đã thanh toán thẳng sang `Cho hoan tien`; không có trạng thái/thực thể riêng thể hiện “yêu cầu hủy đang chờ Admin xét”. Hệ thống cũng không có API riêng cho “từ chối yêu cầu hủy”.

Ngoại lệ giải phóng số lượng voucher khi hủy đơn chưa thanh toán cũng chưa được triển khai trong `cancelOrder()`.

### 3.6. Tìm kiếm sai khi dữ liệu có nhiều trang — mức SAI

Trong `findAdminOrders()`:

1. Query lấy một trang trước bằng `.range(...)` tại `order.repository.js:260-273`.
2. Sau đó mới lọc `search`, `paymentStatus`, `voucherCodeStatus` trong bộ nhớ tại dòng 322-336.
3. `total` vẫn trả count của query chưa áp dụng các bộ lọc trên tại dòng 338.

Hậu quả:

- Đơn phù hợp ở trang khác không được tìm thấy.
- Trang hiện tại có thể báo “Không tìm thấy đơn hàng phù hợp” dù toàn hệ thống có kết quả.
- Tổng số và tổng số trang không đúng với điều kiện tìm kiếm.
- Search không xét `customerEmail`/tài khoản khách hàng; chỉ xét ID, tên khách và tên voucher/đối tác của item đầu tiên (`order.repository.js:295-329`).

Điều này chưa khớp bước 5–8 và A8 của luồng cơ bản.

### 3.7. Audit log chưa đủ yêu cầu — mức THIẾU/MỘT PHẦN

Phần đã có:

- Hủy, hoàn tiền, từ chối hoàn tiền và cấp lại mã đều có lời gọi `auditLogService.log()` trong service.
- API `/admin/orders/:id/logs` và tab “Nhật ký quản trị” đã có.

Phần thiếu hoặc sai:

- Mở/kiểm tra chi tiết đơn hàng không ghi log, trong khi bước 16 và NFR-06 yêu cầu log thao tác kiểm tra.
- Log hủy/hoàn tiền/từ chối không lưu trạng thái trước và sau (`order.service.js:96-138`).
- Log cấp lại chỉ lưu `newCode`, không lưu mã cũ (`order.service.js:141-151`).
- Log cấp lại dùng `targetType: 'voucher_mua'`, trong khi API log của đơn chỉ query `doi_tuong = 'donhang'` (`order.service.js:33-40`), nên log cấp lại không xuất hiện trong tab log của đơn.
- Không ghi kết quả gửi email vì chưa có luồng gửi.
- Thao tác thất bại không được ghi log kết quả `That bai`.
- Audit được ghi sau thay đổi dữ liệu và không cùng transaction, nên khi audit lỗi dữ liệu có thể đã đổi.

## 4. Bảng đối chiếu đầy đủ UC-ADM-04

Quy ước:

- **ĐỦ:** Code hiện tại đáp ứng phần yêu cầu được nêu.
- **MỘT PHẦN:** Có khung hoặc một phần luồng nhưng còn thiếu điều kiện/dữ liệu.
- **THIẾU:** Chưa có logic cần thiết.
- **SAI/CHẶN:** Có code nhưng chạy sai nghiệp vụ hoặc bị schema chặn.

| Yêu cầu UC-ADM-04 | Hiện trạng code | Đánh giá |
|---|---|---|
| Chỉ Admin đã đăng nhập được truy cập | Tất cả admin order route đều dùng `authenticateMiddleware` và `authorizeMiddleware(JWT_ROLES.ADMIN)` tại `admin-order.routes.js:9-70` | **ĐỦ ở backend** |
| Bảo vệ route frontend | `/admin` dùng `ProtectedRoute` trong production; tuy nhiên development bypass mọi role tại `ProtectedRoute.jsx:5-12` | **MỘT PHẦN** |
| Hiển thị danh sách đơn hàng | Có API phân trang và bảng danh sách tại `AdminOrdersPage.jsx:504-607` | **ĐỦ phần khung** |
| Tra cứu theo mã đơn, tên/tài khoản khách, voucher, đối tác | Có tìm ID, tên khách, voucher/đối tác đầu tiên; thiếu tài khoản/email, sai khi phân trang và chưa xét mọi item | **MỘT PHẦN** |
| A8 không có kết quả và giữ khu vực tìm kiếm | UI hiển thị “Không tìm thấy đơn hàng phù hợp” và không xóa filter; nhưng backend có thể kết luận rỗng sai do lọc sau phân trang | **MỘT PHẦN** |
| Hiển thị chi tiết đơn, khách, voucher, đối tác | Có các trường chính; với đơn nhiều voucher/đối tác, phần tổng quan chỉ lấy item đầu tiên làm voucher/đối tác chính (`order.repository.js:398-407`) | **MỘT PHẦN** |
| Phân biệt trạng thái đơn/thanh toán/voucher code | UI có khu vực riêng, nhưng mapping payment/voucher dùng sai bộ trạng thái so với DB | **SAI/CHẶN** |
| Lịch sử thanh toán riêng | Có truy xuất mọi `thanhtoan` và tab riêng (`order.repository.js:100-121`, `422-429`; UI dòng 346-365) | **MỘT PHẦN** vì mapping trạng thái sai |
| Lịch sử sinh và gửi voucher code | Chỉ đọc `lssinhma`; phát hành lần đầu không insert `lssinhma`, không có dữ liệu/lịch sử gửi email | **THIẾU** |
| Chỉ xem/kiểm tra thì không đổi dữ liệu | API GET danh sách/chi tiết không update dữ liệu | **ĐỦ** |
| Ghi audit khi kiểm tra đơn | Không có log trong `getAdminOrderById()` hoặc controller GET | **THIẾU** |
| Hủy đơn đã thanh toán nhưng phát hành code thất bại | Có API update `Cho hoan tien`, nhưng thiếu UI, thiếu kiểm tra payment/code, thiếu khóa/idempotency | **THIẾU/MỘT PHẦN** |
| Lưu lý do hủy | Repository có `ly_do_huy`; frontend có kiểm tra reason trong handler nhưng modal không được render và backend không validate | **MỘT PHẦN** |
| Hủy đơn chưa thanh toán phải giải phóng số lượng giữ | `cancelOrder()` không có logic tồn kho/giải phóng số lượng | **THIẾU** |
| Hoàn tiền chỉ khi đơn ở `Cho hoan tien` | UI chỉ hiện nút với trạng thái đó; backend không kiểm tra lại | **MỘT PHẦN** |
| Vô hiệu mã trước khi hoàn tất hoàn tiền | Code đổi đơn sang `Da hoan tien` trước rồi mới update mã; trạng thái `disabled` còn sai schema và error bị bỏ qua | **SAI/CHẶN** |
| Ghi nhận hoàn tiền mô phỏng | Có insert `hoantien`, nhưng dùng `completed`, tìm payment bằng `success`, rồi dùng `refunded_sim`; đều lệch schema | **SAI/CHẶN** |
| Từ chối hoàn tiền và bắt buộc lý do | Có UI + API; backend không validate reason, insert `rejected` sai schema và chưa gửi thông báo khách hàng | **MỘT PHẦN/SAI** |
| Gửi thông báo kết quả từ chối cho khách | Không có email/notification trong `rejectRefund()` (`order.repository.js:562-578`) | **THIẾU** |
| Cấp lại mã chỉ cho đơn thanh toán thành công và mã lỗi | UI có ý định kiểm tra nhưng dùng sai status; backend không kiểm tra; `orderId` trên URL bị bỏ qua | **SAI/CHẶN** |
| Hủy/vô hiệu mã cũ trước khi cấp mã mới | Có câu lệnh update nhưng dùng `disabled` sai schema, không kiểm tra error và không transaction | **SAI/CHẶN** |
| Sinh mã mới duy nhất từ kho | Dùng `Math.random`, dựa vào UNIQUE của DB nhưng không retry collision; không dùng service phát hành đã có cơ chế retry | **MỘT PHẦN** |
| Trường hợp chưa có mã vẫn cấp lại được | UI cho phép `not_issued`, nhưng handler bắt buộc `selectedOrder.codes[0].id` (`AdminOrdersPage.jsx:178-181`), nên không có mã cũ thì không thể xử lý | **SAI/CHẶN** |
| Cho chọn đúng mã lỗi trong đơn có nhiều mã | UI luôn lấy `codes[0]`, không cho chọn mã gặp lỗi | **THIẾU** |
| Gửi mã mới qua email | Không có lời gọi mailer/email | **THIẾU** |
| Lưu mã cũ, mã mới và kết quả gửi | `lssinhma` có mã cũ/mới nhưng không có kết quả gửi; audit chỉ có mã mới | **MỘT PHẦN** |
| Khi gửi mail lỗi không tự sinh thêm mã | Chưa có luồng gửi mail nên chưa có xử lý ngoại lệ này | **THIẾU** |
| Khi update trạng thái lỗi phải giữ dữ liệu cũ | Không có transaction/rollback; nhiều lệnh còn bỏ qua error | **SAI/CHẶN** |
| Khi vô hiệu mã lỗi phải dừng hoàn tiền/cấp lại | Code không kiểm tra error update mã và vẫn có thể chạy bước sau | **SAI/CHẶN** |
| Audit lỗi thì không lưu thay đổi | Audit strict chỉ throw sau khi dữ liệu đã đổi, không rollback | **SAI/CHẶN** |
| Loading khi tải dữ liệu | Có loading cho danh sách và chi tiết | **ĐỦ** |
| Chống bấm lặp thao tác hủy/hoàn tiền/cấp lại | Không có mutation loading/disabled/idempotency key; các nút có thể bấm nhiều lần | **THIẾU** |
| Lịch sử tải không làm gián đoạn toàn giao diện | Lịch sử nằm trong detail; tuy nhiên detail và order log đang tải tuần tự trong cùng một `try`, chưa tách lỗi độc lập | **MỘT PHẦN** |
| Giao diện responsive | Có `sm`, `md`, `flex-wrap`, `overflow-x-auto`; bảng có cuộn ngang | **ĐỦ phần cơ bản** |
| Các thao tác có bước xác nhận | Hoàn tiền, từ chối hoàn tiền và cấp lại có modal; hủy đơn không có modal hiển thị | **MỘT PHẦN** |
| Hiển thị kết quả rõ ràng | Có toast; nhưng có thể báo thành công dù DB phụ thuộc thất bại do repository bỏ qua error | **SAI/CHẶN** |
| Nhật ký đủ actor/time/order/reason/before/after/old-new/send result | Schema log có actor/time; payload UC-ADM-04 hiện thiếu nhiều trường và thiếu log thất bại | **MỘT PHẦN** |

## 5. Điểm mâu thuẫn ngay trong đặc tả cần xác nhận

Đặc tả UC-ADM-04 có một chỗ không thống nhất:

- Hậu điều kiện mục 5a ghi: khi từ chối hoàn tiền, **giữ nguyên trạng thái đơn hàng**.
- Luồng A14b2 lại ghi: cập nhật đơn thành **“Hủy yêu cầu hoàn tiền”**.

Code hiện tại chọn cách của A14b2 tại `order.repository.js:562-564`. Trước khi sửa nên xác nhận với người phụ trách nghiệp vụ xem trạng thái đúng là giữ `Cho hoan tien`, trở lại trạng thái trước đó, hay chuyển `Huy yeu cau hoan tien`.

## 6. Rủi ro hiệu năng

`findAdminOrders()` đang có mô hình N+1 query:

- Mỗi đơn gọi thêm truy vấn tài khoản, người dùng, thanh toán, hoàn tiền, chi tiết đơn và voucher code.
- Mỗi item lại gọi thêm voucher, `voucher_cn`, chi nhánh và hồ sơ doanh nghiệp (`order.repository.js:10-123`, `278-320`).

Với 10 đơn và nhiều item, một lần tải danh sách có thể phát sinh hàng chục đến hàng trăm request Supabase. Điều này chưa bảo đảm NFR-01 khi dữ liệu tăng. Cần dùng join/batch/RPC hoặc view tổng hợp, đồng thời áp dụng filter và count ngay trong database.

## 7. Thứ tự ưu tiên đề xuất để hoàn thiện

1. **P0:** Chuẩn hóa toàn bộ trạng thái bằng các constant hiện có (`order-status.js`, `payment-status.js`, `issued-voucher-status.js`) và làm cho schema, seed, backend, frontend cùng một bộ giá trị.
2. **P0:** Đưa hủy/hoàn tiền/vô hiệu mã/cấp lại mã/audit bắt buộc vào transaction hoặc Supabase RPC; mọi bước phải kiểm tra lỗi.
3. **P0:** Thêm state-transition validation ở backend và kiểm tra `orderId` ↔ `maVoucherMua`; bắt buộc lý do.
4. **P0:** Sửa hoàn tiền theo đúng thứ tự: khóa/vô hiệu mã thành công → ghi hoàn tiền thành công → chuyển đơn `Da hoan tien` → ghi audit; thất bại phải rollback.
5. **P1:** Hoàn thiện luồng cấp lại: hỗ trợ không có mã cũ, chọn đúng mã lỗi, sinh mã bằng repository có collision retry, tạo QR, gửi email, lưu kết quả gửi và audit mã cũ/mới.
6. **P1:** Render luồng hủy đơn trên Admin và tách rõ “yêu cầu hủy”, “Admin chấp nhận”, “Admin từ chối”.
7. **P1:** Sửa tìm kiếm/lọc/phân trang tại database và search cả tài khoản/email, tất cả voucher/đối tác trong đơn.
8. **P1:** Bổ sung audit đầy đủ cho xem chi tiết, hủy, hoàn tiền, từ chối, vô hiệu mã, cấp lại và gửi email; log thất bại phải có `That bai`.
9. **P2:** Thêm mutation loading, disable nút, request key/idempotency và tách lỗi tải từng khu vực lịch sử.
10. **P2:** Tối ưu N+1 query bằng batch/join/view/RPC.

## 8. Kết luận cuối cùng

### Phần có thể giữ lại

- Kiến trúc route/controller/service/repository.
- Route backend có xác thực và phân quyền Admin.
- Khung trang danh sách/chi tiết, các tab lịch sử và responsive cơ bản.
- Các API endpoint chính và modal hoàn tiền/từ chối/cấp lại.
- Schema UNIQUE cho `voucher_code` và khung audit log chung.

### Phần bắt buộc sửa/bổ sung trước nghiệm thu

- Đồng bộ trạng thái với database.
- Transaction và rollback.
- Validation chuyển trạng thái tại backend.
- Luồng hủy đơn Admin đầy đủ.
- Luồng cấp lại mã + email + lịch sử gửi.
- Search trước phân trang.
- Audit đầy đủ và nguyên tử.
- Chống thao tác lặp.

**Đánh giá cuối:** UC-ADM-04 mới có khung giao diện/API và một phần truy xuất dữ liệu; các luồng ghi quan trọng chưa an toàn hoặc chưa thể chạy đúng với schema hiện tại. Trạng thái phù hợp để ghi nhận là `CHƯA HOÀN THÀNH`, không phải `ĐẠT`.
